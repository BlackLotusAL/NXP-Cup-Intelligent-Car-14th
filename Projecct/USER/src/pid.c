...
void IncPID_Operation(PID_value *pid)
{
	...
	pid->AI = pid->error;
	pid->AD = (pid->error - 2 * pid->error_1 + pid->error_2);
...
void PosPID_Operation(PID_value *pid)
{
    ...
    //计算中间变量AP,AI,AD;
    pid->AP = pid->error;
    pid->AI = pid->error_sum;
    pid->AD = pid->error - pid->error_1;
...
* @注  意：无
*/
void PID_Init(PID_value *pid, float KP, float KI, float KD, float min, float max)
{
